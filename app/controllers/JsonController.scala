...
@Singleton class JsonController @Inject()(ws: WSClient, val controllerComponents: ControllerComponents) extends BaseController with Logging:
  ...
  def callHello(): Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
    def greetingUrl(name: String): String = s"""http://localhost:9000/return_hello1/$name"""
